...
cooling=("aa" "pv" "av" "l")
BENCHMARK=("sjeng" "gcc" "hmmer" "lbm" "mcf" "sjeng" "soplex")
bench_i=0
temp_i=0
cooling_i=0
freq_i=0
...
do
 while [ ${bench_i} -ne 1 ]
 do
	./hotspot -c ${config_dir}/config/${BENCHMARK[$[bench_i]]}/${BENCHMARK[$[bench_i]]}_vapor_passive.config -f G_HBM_BASE.flp -p ${output_dir2}/${FREQ[$[freq_i]]}/${BENCHMARK[$[bench_i]]}/pv_${worst}.ptrace -model_type grid -grid_layer_file G8M3D_HBM.lcf -detailed_3D on -grid_steady_file test2.grid
